...
<script>
	/*
		bind()
			: 이벤트를 여러개 연결할 때 사용하는 메소드
		unbind()
			: 이벤트를 해제할 때 사용하는 메소드
		※ 주로 메서드체인 형태로 사용해서 여러개의 이벤트를 연결할 수 있다. 
	*/
	...
	{		
		$('#click_change')
			...
			{			
				//JS의 함수를 이용해서 배경색 변경			 
				document.getElementById("click_change").style.backgroundColor="red";
			})
			.bind("mouseout", function()
			{
				//jQuery의 get()메소드를 통해 요소를 선택하고 배경색 변경
				$(this).get(0).style.backgroundColor="white";
			})
			.bind("click", function()
			{
				//text() : JS의 innerText()와 같은 역할의 메소드
				alert($(this).text());
			});
		...
		{
			/*
			해당 코드가 실행되면 #btnClick을 눌러도 아무런 반응이 없게 된다.
			즉, click이벤트가 제거된다. 
			*/
