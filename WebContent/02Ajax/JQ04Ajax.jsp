...
{
	/*
		해당 메소드는 문서의 로드가 끝난 직 후 js파일을 현재 문서에 로드한다.
		즉, .load()메소드와 동일한 역할을 하고 있다.
	*/
	$.ajax(
	{
		url : './common/04JsData.js', //외부에 있는 것을 로드하는중
		type : 'get',
		dataType : "script",
		//성공했을 때의 콜백메소드 : 무기명함수 형태로 정의된다.
		success : function(resData) //성공했을 때의 콜백
		{
			MyAlert("Hello ", "AJAX"); //Myalert함수 호출!
		},
		//실패했을 때의 콜백메소드 : 외부함수 형태로 정의된다.
		error : errFunc //실패했을 때의 콜백
		//호출시 함수명만 명시한다. errFunc() 이렇게 안쓴다!
	});
	...
	$('#ajaxBtn').click(function()
	{
		$.ajax(
		{
			...
			data : //파라미터. JSON으로 이루어져있다
			{
				msg : $(this).text(),
				varStr : "$.ajax()메소드 매우 좋아요!"
			},
			//성공했을 때의 콜백메소드 : 무기명함수 형태로 정의된다.
			//sucFunc(매개변수) 형태로 사용하지 않는다.
			success : sucFunc,
			//실패했을 때의 콜백메소드 : 외부함수 형태로 정의된다.
			error : errFunc
		});
