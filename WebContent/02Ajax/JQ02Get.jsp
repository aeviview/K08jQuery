...
/*
	...
	2. $.get() : HTTP get방식을 통해 서버로부터 데이터를 받을 때 사용하는 메소드
			...
			function(data) //콜백메소드
			{
				...
				*/
				$(data).find("명함").each(function()
				{
					...
					/*
						해당 영역의 내용을 empty()로 삭제한 후,
						append()로 내용을 추가한다. html()과 동일한 기능이 수행된다.
					*/
					$('#xmlDisplay').empty();
...
//얘는 자바스크립스 함수이다 => 위의 제이쿼리 영역안에 포함되어 있으면 작동하지 않는다!
...
<div>	
	...
	<h3>jsp파일에서 읽어오기</h3>
	<button onclick="locationGo('common/02PrintToday.jsp?msg=안녕하세용&varStr=jQuery좋아용');">
		PrintToday.jsp바로가기
	</button>
	<button id="btnJSP">
		JSP결과 읽어오기
